...
export default function App() {
  ...
  // Sync annotate button state when the overlay is closed from the page (Esc / ✕)
  useEffect(() => {
    const onMessage = (message: any) => {
      if (message?.type === 'annotation-state') setAnnotateActive(!!message.active);
    };
    browser.runtime.onMessage.addListener(onMessage);
    return () => browser.runtime.onMessage.removeListener(onMessage);
  }, []);

  const doToggleAnnotate = useCallback(async () => {
    try {
      const tab = await getWebpageTab();
      if (!tab?.id) { log.warn('Annotate: no webpage tab found'); return; }
      const res = await browser.tabs.sendMessage(tab.id, { type: 'toggle-annotation' });
      setAnnotateActive(!!res?.active);
      log.info('Annotate toggled:', res);
    } catch (err) {
      log.error('Annotate toggle failed:', err);
    }
  }, []);
